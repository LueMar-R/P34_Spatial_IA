...
class Affichage(tk.Tk):
    ...
    def __init__(self, largeur, hauteur, array_lieux=False, nb_lieux=25):
        tk.Tk.__init__(self)
    ...
    def draw_graph(self):  #fonction qui dessine le graphe
        self.DIAM_CERCLE = 9
        ...
        for x in range(len(self.liste_lieux)): #boucle for qui dessine les points et écrit le numéro à l'interieur
            if x == 0:
                color = 'red'
            else:
                color = 'silver'
    ...
    # initialisation
        self.premiere_route = self.TSP.chemin_zero #récupérartion de l'ordre de l'heuristique de départ
        self.best_distance = self.TSP.distance_zero #récupérartion de la distance de la route de l'heuristique de départ

        for x in self.premiere_route: #traçage de la première route
            try:
